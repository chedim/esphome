 ...
 */
RTC_DATA_ATTR static uint8_t LoRaMacNwkSKey[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                                 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

/*!
 * AES encryption/decryption cipher application session key
 */
RTC_DATA_ATTR static uint8_t LoRaMacAppSKey[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                                 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
void OnRadioRxDone(uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr) {
  ...
  switch (macHdr.Bits.MType) {
        ...
        if (micRx == mic) {
          ...
          if (g_lora_debug) {
            PRINTF_RAW("Rx1DrOffset:%u Rx2Channel.Datarate:%u ReceiveDelay1:%u\r\n", LoRaMacParams.Rx1DrOffset,
                       LoRaMacParams.Rx2Channel.Datarate, (unsigned int) LoRaMacParams.ReceiveDelay1);
          }
#endif
          // Apply CF list
    ...
    case FRAME_TYPE_DATA_UNCONFIRMED_DOWN: {
      ...
      if (address != LoRaMacDevAddr) {
        ...
        if ((macHdr.Bits.MType != FRAME_TYPE_DATA_UNCONFIRMED_DOWN) || (fCtrl.Bits.Ack == 1) ||
      ...
      if (isMicOk == true) {
        ...
        if (multicast == 1) {
          ...
          if ((curMulticastParams->DownLinkCounter == downLinkCounter) && (curMulticastParams->DownLinkCounter != 0)) {
        ...
        if (((size - 4) - appPayloadStartIndex) > 0) {
          ...
          if (port == 0) {
            ...
            if ((fCtrl.Bits.FOptsLen == 0) && (multicast == 0)) {
              LoRaMacPayloadDecrypt(payload + appPayloadStartIndex, frameLen, nwkSKey, address, DOWN_LINK,
                                    downLinkCounter, LoRaMacRxPayload);
            ...
            LoRaMacPayloadDecrypt(payload + appPayloadStartIndex, frameLen, appSKey, address, DOWN_LINK,
                                  downLinkCounter, LoRaMacRxPayload);
  ...
  CheckToDisableAckTimeout(NodeAckRequested, LoRaMacDeviceClass, McpsConfirm.AckReceived, AckTimeoutRetriesCounter,
                           AckTimeoutRetries);
...
static void OnMacStateCheckTimerEvent(void) {
  ...
  if (LoRaMacFlags.Bits.MacDone == 1) {
    ...
    if ((LoRaMacFlags.Bits.MlmeReq == 1) || ((LoRaMacFlags.Bits.McpsReq == 1))) {
      ...
      if ((LoRaMacConfirmQueueIsCmdActive(MLME_BEACON_ACQUISITION) == true) && (LoRaMacFlags.Bits.McpsReq == 0)) {
  ...
  switch (LoRaMacDeviceClass) {
    case CLASS_A: {
      ...
      if (deviceClass == CLASS_C) {
        ...
        RegionComputeRxWindowParameters(LoRaMacRegion, LoRaMacParams.Rx2Channel.Datarate, LoRaMacParams.MinRxSymbols,
                                        LoRaMacParams.SystemMaxRxError, &RxWindow2Config);
        OpenContinuousRx2Window();
...
static bool ValidatePayloadLength(uint8_t lenN, int8_t datarate, uint8_t fOptsLen) {
  ...
  // if ( ((( payloadSize > maxN ) && (fOptsLen != 0) && (fOptsLen <= maxN)) || ( payloadSize <= maxN )) && (
  // payloadSize <= LORAMAC_PHY_MAXPAYLOAD ) ) {
  if ((((payloadSize > maxN) && (fOptsLen != 0) && (lenN <= maxN) && (fOptsLen <= maxN)) || (payloadSize <= maxN)) &&
      (payloadSize <= LORAMAC_PHY_MAXPAYLOAD)) {
    return true;
  }
  return false;
  ...
  while (macIndex < commandsSize) {
    ...
    switch (payload[macIndex++]) {
      ...
      case SRV_MAC_LINK_ADR_REQ: {
        ...
        status = RegionLinkAdrReq(LoRaMacRegion, &linkAdrReq, &linkAdrDatarate, &linkAdrTxPower, &linkAdrNbRep,
...
LoRaMacStatus_t Send(LoRaMacHeader_t *macHdr, uint8_t fPort, void *fBuffer, uint16_t fBufferSize) {
  ...
  if (g_lora_debug == true) {
    ...
    PRINTF_RAW("fCtrl rfu:%d Adr:%d AdrAckReq:%d fCtrl.Bits.Ack:%d\r\n", fCtrl.Bits.FPending, fCtrl.Bits.Adr,
               fCtrl.Bits.AdrAckReq, fCtrl.Bits.Ack);
  }
#endif
  return status;
}
...
static LoRaMacStatus_t ScheduleTx(void) {
  ...
  RegionComputeRxWindowParameters(LoRaMacRegion,
                                  ...
                                  LoRaMacParams.MinRxSymbols, LoRaMacParams.SystemMaxRxError, &RxWindow1Config);
...
LoRaMacStatus_t PrepareFrame(LoRaMacHeader_t *macHdr, LoRaMacFrameCtrl_t *fCtrl, uint8_t fPort, void *fBuffer,
  ...
  switch (macHdr->Bits.MType) {
    ...
    case FRAME_TYPE_DATA_UNCONFIRMED_UP:
      ...
      fCtrl->Bits.AdrAckReq = RegionAdrNext(LoRaMacRegion, &adrNext, &LoRaMacParams.ChannelsDatarate,
      ...
      MacCommandsBufferToRepeatIndex =
          ParseMacCommandsToRepeat(MacCommandsBuffer, MacCommandsBufferIndex, MacCommandsBufferToRepeat);
      if (MacCommandsBufferToRepeatIndex > 0) {
        MacCommandsInNextTx = true;
      }
      ...
      LoRaMacComputeMic(LoRaMacBuffer, LoRaMacBufferPktLen, LoRaMacNwkSKey, LoRaMacDevAddr, UP_LINK, UpLinkCounter,
                        &mic);
...
LoRaMacStatus_t LoRaMacInitialization(LoRaMacPrimitives_t *primitives, LoRaMacCallback_t *callbacks,
  ...
  if ((primitives->MacMcpsConfirm == NULL) || (primitives->MacMcpsIndication == NULL) ||
      (primitives->MacMlmeConfirm == NULL) || (primitives->MacMlmeIndication == NULL)) {
    return LORAMAC_STATUS_PARAMETER_INVALID;
  }
...
LoRaMacStatus_t LoRaMacQueryTxPossible(uint8_t size, LoRaMacTxInfo_t *txInfo) {
  ...
  */
  ...
  while (ValidatePayloadLength(size, datarate, fOptLen) == false) {
    ...
    printf(
        "Payload length(%d) and fOptLen(%d) exceed max size(%d for current datarate DR %d), set datarate to Dr %d\r\n",
